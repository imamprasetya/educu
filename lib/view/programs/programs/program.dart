...
class _ProgramScreenState extends State<ProgramScreen> {
  ...
  Future<void> loadPrograms() async {
    ...
    final resultMaps = await DBHelper.getProgramsByUser(userId!);
    final result = resultMaps.map((e) => ProgramModel.fromMap(e)).toList();

    // load progress for each program
    Map<int, double> pMap = {};
    for (var p in result) {
      if (p.id != null) {
        final sessions = await DBHelper.getSessions(p.id!);
        if (sessions.isEmpty) {
          pMap[p.id!] = 0.0;
        } else {
          int completed = sessions.where((s) => s['completed'] == 1).length;
          pMap[p.id!] = completed / sessions.length;
        }
      }
    }
