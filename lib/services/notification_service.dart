...
class NotificationService {
  ...
  Future<void> scheduleAllNotifications() async {
    ...
    final todaySessions = await FirebaseService.getSessionsByDate(todayStr);
    final incompleteTodaySessions =
        todaySessions.where((s) => s['completed'] != true).toList();

    // ── 2. Get tomorrow's sessions ──
    final tomorrowSessions = await FirebaseService.getSessionsByDate(tomorrowStr);

    // ── 3. Get yesterday's missed sessions ──
    final yesterdaySessions = await FirebaseService.getSessionsByDate(yesterdayStr);
    final missedYesterday =
        yesterdaySessions.where((s) => s['completed'] != true).toList();
    ...
    final incompleteTomorrow =
        tomorrowSessions.where((s) => s['completed'] != true).toList();
